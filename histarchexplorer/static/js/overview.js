// ============================================================
//  OVERVIEW.JS — patched for actual window.entityData structure
// ============================================================
...
(function initGrid() {
  try {
    ...
    if (!gridEl || typeof Muuri === 'undefined') {
      console.warn('⚠️ Muuri not available or no grid element found.');
      return;
    }
    ...
    console.error('Muuri init failed:', err);
  }
})();
...
function pickDescription(descObj) {
  ...
  const preferred = ['CURRENT_LANGUAGE', 'en', 'de', 'fr', 'it'];
...
function renderHierarchyButtons() {
  ...
  const btn = h('button', {
    ...
    title: 'Hierarchy (dummy)',
    disabled: ''
  }, [
    h('i', { class: 'bi bi-diagram-3 me-1' }),
    h('span', { text: 'Subentities' })
  ]);
  host.appendChild(h('div', { class: 'hierarchy-button' }, [btn, h('div', { class: 'hierarchy-line' })]));
  relayout(10);
}
...
function renderEntityCard(entity, mainImage) {
  ...
  if (['group', 'person'].includes(sysClass) && mainImage?.url)
    ...
    avatar = `<img src="${systemIconPath(entity.system_class)}" alt="${entity.system_class}">`;

  const types = formatTypeTitles(entity.types);
  const dateStr = dateTemplate(entity.start, entity.end);

  host.innerHTML = `
    <div class="main-entity-ellipse main-entity-ellipse--${(entity.view_class || 'entity')}s">
      <div class="entity-card__icon">${avatar}</div>
      <div class="hierarchy-card-label mb-0">${entity.title || ''}</div>
      ${types ? `<div class="entity-type mb-0"><p>${types}</p></div>` : ''}
...
function renderDescription(entity) {
  const tile = document.getElementById('tile-description');
  if (!tile) return;
  const text = pickDescription(entity?.description);
  if (!text) return;
  document.getElementById('js-description').textContent = text;
  tile.hidden = false;
  relayout(10);
}
...
function renderModelViewer(url, alt = '') {
  const wrapper = h('div', { class: 'model-wrapper', style: 'height:300px;position:relative;' }, [
    h('div', { class: 'spinner' }),
    h('model-viewer', {
      src: url, alt,
      'camera-controls': '', ar: '',
...
function renderFileBlock(file, host) {
  switch (file.render_type) {
    case '3d_model':
      host.appendChild(renderModelViewer(file.url, file.title || '3D Model')); break;
    case 'video':
      host.appendChild(h('video', { controls: '', class: 'w-100 rounded', src: file.url })); break;
    case 'pdf':
      host.appendChild(h('a', { href: file.url, target: '_blank', class: 'btn btn-outline-secondary', text: file.title || 'Open PDF' })); break;
    default:
...
function startOverview() {
  ...
  // 💡 REBUILD Muuri grid once all hidden tiles are visible
  setTimeout(() => {
    document.querySelectorAll('.grid-muuri .item[hidden]').forEach(el => el.hidden = false);
    console.log('♻️ Reinitializing Muuri grid');
    try { if (window.overviewGrid) window.overviewGrid.destroy(); } catch(e){}
    window.overviewGrid = new Muuri('.grid-muuri', {
      layout: { fillGaps: true },
      dragEnabled: false
    });
...
if (document.readyState === 'loading')
  ...
  requestAnimationFrame(startOverview);
