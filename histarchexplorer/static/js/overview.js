...
function relayout(delay = 0) {
    ...
    if (delay > 0) setTimeout(doIt, delay);
...
// jetzt erst auf Bilder warten und dann layouten
    relayoutOnImages(grid);

// optional: falls mediaGrid existiert, sicher refreshen
    setTimeout(() => {
        if (window.mediaGrid) window.mediaGrid.refreshItems().layout();
    }, 0);


// ---- helper (kann auch weiter oben stehen) ----
    function relayoutOnImages(container = document) {
